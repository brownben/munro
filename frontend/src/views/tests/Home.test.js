...
test('Shows Admin Buttons When Logged In', () => {
  ...
  expect(
    wrapper.find('.actions').findAll('router-link-stub').at(0).text()
  ).toBe('Create New League')
  expect(
    wrapper.find('.actions').findAll('router-link-stub').at(1).text()
  ).toBe('Manage Competitors')
})
